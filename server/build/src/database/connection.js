"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.databaseConnection = exports.redisClient = void 0;
const mongoose_1 = require("mongoose");
const ioredis_1 = __importDefault(require("ioredis"));
exports.redisClient = new ioredis_1.default();
const databaseConnection = (options) => __awaiter(void 0, void 0, void 0, function* () {
    mongoose_1.connection.on('open', () => {
        console.log('Database status    : Connected');
        console.log('Database Name      : ' + mongoose_1.connection.name);
    });
    mongoose_1.connection.on('error', (error) => {
        console.log('Database status    : Error');
        console.log('Error Message      : ' + error.message);
    });
    (0, mongoose_1.set)('allowDiskUse', true);
    (0, mongoose_1.set)('strictQuery', true);
    yield (0, mongoose_1.connect)(options.url);
});
exports.databaseConnection = databaseConnection;
//# sourceMappingURL=connection.js.map